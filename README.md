...
- 游客与每个账号都会自动获得一份**内置初始招聘信息**（字节跳动、搜狐畅游、汇川技术、4399、好未来、科大讯飞、联想、叠纸、博西家电、亿联网络、新东方、帆软等），可作为起步参考并自由编辑；已有记录的空间不会被重复导入。
- **注册/登录是可选功能**：产生留存意愿（想跨设备同步）时可注册登录，**游客期间的数据会自动合并到账号**，登录后任意设备同步同一份数据。
...
2. 拉取代码后执行上述安装与构建步骤。
3. **在服务器上创建 `server/.env`**（该文件被 git 忽略，必须手动创建）：
   复制 `server/.env.example` 为 `server/.env`，至少填写管理员账号与密码，否则无法登录：
   ...
   也可以用系统环境变量（`ADMIN_USERNAME`、`ADMIN_PASSWORD`）注入，优先级高于 `.env`。
4. 用 `pm2` 守护后端：`pm2 start server/index.js --name mytools`。
5. 配置 Nginx 反向代理：
