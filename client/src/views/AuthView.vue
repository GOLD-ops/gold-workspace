    ...
    <div class="auth-card">
      ...
      <form class="auth-form" @submit.prevent="submit">
        ...
        <template v-else-if="mode === 'register'">
          ...
          <div v-if="inviteRequired !== '0'" class="auth-row-field">
            <label>邀请码</label>
            <div class="auth-input-wrap">
              <input
                v-model="inviteCode"
                class="tk-input"
                :placeholder="inviteRequired === '1' ? '请输入邀请码' : '选填，如已开启邀请注册'"
                autocomplete="off"
              />
            </div>
          </div>
...
const inviteRequired = ref(null)
let countdownTimer = null
let messageTimer = null

onMounted(async () => {
  try {
    const r = await api('/api/auth/invite-config')
    inviteRequired.value = r.invite_required || '0'
  } catch {
    inviteRequired.value = null
  }
})
...
function flash(kind, text) {
  ...
  else success.value = text
  messageTimer = setTimeout(clearMessages, 3000)
}
...
async function sendCode() {
  ...
  try {
    ...
    flash('success', r.message || '验证码已发送')
    startCountdown()
  } catch (e) {
...
async function submit() {
  ...
  if (mode.value === 'register') {
    if (!email.value.trim() || !code.value.trim() || !password.value) {
      flash('error', '请完整填写邮箱、验证码和密码')
      return
    }
    if (inviteRequired.value === '1' && !inviteCode.value.trim()) {
      flash('error', '请填写邀请码')
      return
    }
  } else if (mode.value === 'reset') {
    if (!email.value.trim() || !code.value.trim() || !password.value) {
      flash('error', '请完整填写邮箱、验证码和新密码')
      return
    }
  } else if (loginMethod.value === 'code') {
    if (!email.value.trim() || !code.value.trim()) {
      flash('error', '请填写邮箱和验证码')
      return
    }
  } else if (!EMAIL_RE.test(email.value.trim()) || !password.value) {
    flash('error', '请输入正确的邮箱和密码')
    return
  }
